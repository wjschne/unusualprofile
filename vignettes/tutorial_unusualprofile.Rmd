...
With positively correlated variables, the profile's elevation typically matters much more than the profile shape. Thus, when considerating how unusual a set of scores is, we might want to distinguish overall elevation from relative profile shape when we quantify how unusual a profile is. One way to define the profile elevation is to create a composite score from the sum of profile variables. All profiles that produce the same composite score are define to have the same profile elevation. Ideally, the profile variables and the composite score are re-scaled to have the same metric---preferably the *z*-score metric.
