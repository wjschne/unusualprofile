...
.caption {text-align:right; font-size:0.9rem}
...
fig <- function() cnt_fig <<- cnt_fig + 1

```

The unusualprofile package can identify multivariate outliers conditional on predictors [@ji2018].
...
```{r univariate, echo= FALSE, fig.cap=glue("<em>Figure {fig()}</em>. A histogram with a univariate outlier.")}
...
```{r multivariate, echo=FALSE, warning=FALSE, fig.cap=glue("<em>Figure {fig()}</em>. A scatterplot with a multivariate outlier.")}
set.seed(5)
...
```{r multivariate_outlier_2, echo= FALSE, fig.cap=glue("<em>Figure {fig()}</em>. A two-variable violin plot with a multivariate outlier.")}
k <- 400
head(d_mo, k) %>% 
  rowid_to_column("id") %>%
...
```{r multivariate_outlier_4, echo=FALSE, fig.cap=glue("<em>Figure {fig()}</em>. A four-variable violin plot with a multivariate outlier.")}
...
The simplest (but ultimately unsatisfying) way to measure a profile's rarity is with the Euclidean distance. A multidimensional extension of the Pythagorean Theorem, the *Euclidean distance* is the square root of the sum of the squared differences on each dimension from some reference point. The reference point of interest is usually the vector of means from each variable---the *centroid*. The Euclidean distance of point *p*~1~ = (1,1,−1,1) to the centroid *p*~2~ = (0,0,0,0) is
...
```{r one_dimensional, echo = FALSE, fig.align='center', fig.cap=glue("<em>Figure {fig()}</em>. A simple model with standardized loadings"), out.width=300}
...
```{r conditional_dist, echo = FALSE, fig.cap=glue("<em>Figure {fig()}</em>. Conditional distributions for people with a composite score of 1.")}
...
d %>% 
  ...
  geom_normalviolin(aes(mu = mu, sigma = sigma, x = Variable), data = tibble(Variable = factor(c("x1","x2","x3","x4")), mu = 0, sigma = 1, X = 1), alpha = 0.3, face_right = F)

```

# A Worked Example

```{r standardized_scores, echo=FALSE}
...
```{r, echo = F, fig.cap=glue("<em>Figure {fig()}</em>. Example profile in a standard multivariate normal distribution.")}
...
```{r, ref.label = 'standardized_scores'}
...
The simstandard package can create simulated multivariate normal data from a structural equation model. The first step is to specify the model using [lavaan syntax](http://lavaan.ugent.be/tutorial/syntax1.html):
...
```{r}
# Convert X to row matrix, and add the composite score
d <- t(X) %>% 
...
cm <- cond_maha(data = d,
                R = R_all,
                v_dep = v_observed,
                v_ind_composites = v_composite)
cm
```

```{r, fig.cap=glue("<em>Figure {fig()}</em>. A profile of z-scores in the context of population distributions (darker gray) and conditional distributions (lighter gray) controlling for overall composite score")}
plot(cm, family = myfont)
```

Here we can see that the profile is more unusual than `r round(100*cm$dCM_p, 0)` of profiles with the same elevation (i.e., a composite score of *z* = `r round(d$X_Composite, 2)`).
...
```{r matrix_loadings}
# Factor loadings
lambda <- matrix(c(0.95, 0.90, 0.85, 0.60), 
                 dimnames = list(paste0("X_", 1:4),
                                 "X"))
...
dimnames(R_all) <- list(v_names, v_names)
R_all
```
...
z_C <- sum(X - mu_X) / sqrt(sum(Sigma_X))
```
...
```{r}
d_mc <- sqrt(t(X - X_hat) %*% solve(R_X) %*% (X - X_hat))[1,1]
d_mc
```
...
Factor scores are calculated using Thurstone's method [@thurstone1935vectors]:
...
d_case <- tibble(
  ...
  simstandard::add_composite_scores(m = m_Reading)
...
```{r, echo = FALSE}
dCM_composite <- cond_maha(data = d_case, 
          R = R, 
          v_dep = Reading_Composite, 
          v_ind = Cognitive_Composite) 
d_case %>% 
...
Here we see that the Gc profile is somewhat unusual, but not particularly so. Among people with a composite Gc of `r round(d_case$Gc_Composite * 15 + 100)`, this profile is more unusual than `r round(dCM_Gc$dCM_p * 100)`% of profiles. From the plot below, Gc2 is a little high (standardized residual = `r round(dCM_Gc$d_dep_residuals_z$Gc2, 2)`) and Gc3 is a little low (standardized residual = `r round(dCM_Gc$d_dep_residuals_z$Gc3, 2)`).

```{r, echo = FALSE}
bind_rows(d_case %>% select(Gc1:Gc3) * 15 + 100,
  ...
  add_row(Variable = "Gc",
          Predicted = 100, 
          Score = d_case$Gc_Composite * 15 + 100 ,
          SEE = 15,
          Role = "Unconditional") %>% 
