...
```{r worked-plot-code, echo=TRUE, eval=FALSE}
plot(cm)
```

```{r cond-maha, fig.cap="A profile of z-scores in the context of population distributions (darker gray) and conditional distributions (lighter gray) controlling for overall composite score.", echo=FALSE}
plot(cm, family = myfont)
```

In Figure&nbsp;\@ref(fig:cond-maha), we can see that the profile is more unusual than `r round(100*cm$dCM_p, 2)`% of profiles with the same elevation (i.e., a composite score of *z* = `r round(d$X_Composite, 2)`). The conditional Mahalanobis distance of the dependent variables is `r round(cm$dCM, 2)`, which is a `r round(100 * cm$distance_reduction)`% smaller than the unconditional Mahalanobis distance of the dependent variables `r round(cm$dM_dep, 2)`.
...
We can use [lavaan syntax](http://lavaan.ugent.be/tutorial/syntax1.html) to specify the standardized coefficients of our model.
...
```{r cond-reading-display, echo = TRUE, eval = F}
plot(dCM)
```

```{r cond-reading, fig.cap="Conditional Distributions for Reading, Controlling for Cognitive Predictors", echo=FALSE}
plot(dCM, family = myfont)
```
...
```{r composite-condmaha-display, echo=TRUE, eval=FALSE}
...
```{r composite-condmaha, echo=FALSE, fig.cap="Conditional Distributions for Reading Composites, Controlling for Cognitive Composites"}
# Conditional Reading Profile
cond_maha(data = d_case, 
          R = get_model_implied_correlations(m_reading, composites = TRUE), 
          v_dep = v_reading_composite, 
...
Because factor scores are estimates, they are presented with confidence intervals in Figure&nbsp;\@ref(fig:factor-scores).
...
d_factor_scores <- d_case %>% 
  ...
  mutate(Variable = if_else(str_ends(Variable, "_FS"), Variable, str_remove(Variable, "_FS"))) %>% 
  separate(Variable, c("Variable", "Type")) %>% 
  pivot_wider(id_cols = Variable, names_from = Type, values_from = Score) %>% 
  mutate(se = simstandard::sim_standardized_matrices(m_reading)$Coefficients$factor_score_se[1:4] * 15)
...
For example, to make factor scores for the reading model (See Figure&nbsp;\@ref(fig:reading-model)), one would use the factor-score coefficients presented in Figure&nbsp;\@ref(fig:factor-score-coefficients). The matrix of factor-score coefficients can be obtained like so:

```{r, eval=FALSE}
simstandard::sim_standardized_matrices(m_reading)$Coefficients$factor_score
```


```{r, factor-score-coefficients, echo=FALSE, fig.cap="Factor score coefficients for the reading model"}

simstandard::sim_standardized_matrices(m_reading)$Coefficients$factor_score[,1:4] %>% 
  as.data.frame() %>% 
...
Factor scores generally have a smaller standard deviation than the latent scores they estimate. Instead, their standard deviation is proportional to their validity. If a factor score measures a latent score poorly, it strongly regresses to the mean, resulting in a smaller variance than that of the latent variable. 

```{r}
R_factor <- get_model_implied_correlations(m_reading, observed = T, factor_scores = T)

cond_maha(d_case, R = R_factor, sigma = c(rep(1, 12), sim_standardized_matrices(m_reading)$Coefficients$composite_score_validity), v_dep = colnames(R_factor)[1:12],
          v_ind_composites = sim_standardized_matrices(m_reading)$v_names$v_factor_score[1:4]) %>% 
  plot()
sim_standardized_matrices(m_reading)
