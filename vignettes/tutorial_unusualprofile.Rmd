...
bmatrix <- function(x) {
  paste0("\\begin{bmatrix}\n", paste0(apply(x,1,paste0, collapse = " & "), collapse = "\\\\\n"), "\n\\end{bmatrix}")
}
...
A univariate outlier is far from most of the other scores in a distribution. You can easily find spot a large outlier in a histogram:
...
Scatterplots are great for inspecting multivariate outliers with a small number of variables. Unfortunately, scatterplots can only display 2 or three variables at a time. A different way to view multivariate data is to show each case as a profile of scores connected by lines. In the plot below, most of the lines are nearly flat---highly correlated variables with the same means and standard deviations will generally produce flat profiles. The multivariate outlier, in red, is clearly not flat.
...
The simplest (but ultimately unsatisfying) way to measure unusuallness is with the Euclidean distance. A multidimensiontal extension of the Pythagorean Theorem, the *Euclidean distance* is the square root of the sum of the squared differences on each dimension from some reference point. The reference point of interest is usually the vector of means from each variable---the *centroid*. The Euclidean distance of point *p*~1~ = (1,1,&minus;1,1) to the centroid *p*~2~ = (0,0,0,0)  is
...
In 1936, P. R. Mahalanobis introduced a variant of the Euclidean distance that accounts for the covariance of the variables. Conceptually, the *Mahalanobis distance* is a Euclidean distance of profile scores if the variables are rotated and rescaled to fit on their principal component axes. Because principal components are always uncorrelated, distances in principal component space always have the same meaning no matter what the relationships are in the original variables. 
...
If the variables in *X* are normally distributed, essentially the Mahalanobis distance is creating principal component scores that are uncorrelated standard normal variates, squaring each score, and then adding row of scores. Adding squared uncorrelated standard normal variates just so happens to be how the &chi;^2^ distribution is made. The degrees of freedom in the &chi;^2^ distribution corresponds to the number of standard normal variates that are squared and summed.

Thus, if there are *k* normally distributed variables in vector *X*, the Mahalanobis distance squared for vector *X* has a &chi;^2^ distriution with *k* degrees of freedom. In mathematical notation:
...
One the disadvantages of the Mahalanobis Distance is that it treats all the dimensions equivalently. For highly correlated variables, the first principal component (or general factor) is of particular importance. We might want to distinguish between cases that are unusual on the first principal component and scores that are unusual on the remaining principal components. 
...
With positively correlated variables, the profile's elevation is so important that we might want to distinguish it from the profile shape when we quantify how unusual a profile is. Suppose that we compare all profiles that have the same elevation but have different profile shapes. One way to define the profile elevation is to create a composite score from the sum of profile variables. All profiles that produce the same composite score are define to have the same profile elevation. Ideally, the profile variables and the composite score are re-scaled to have the same metric---preferably the *z*-score metric.

Imagine that four standardized variables correlate according to the structural model below, and we select a subset of cases in which the profiles have an elevation of 1 (i.e., their composite score has a *z*-score of 1). 


```{tikz, echo = F, out.width=500, fig.align = "center"}
...
\tikzset{latent/.style={circle, minimum size=20mm,inner sep=0mm,font=\huge, text = black, fill = gray},
	error/.style={circle, fill = gray, inner sep=0mm, minimum size=9mm,font=\footnotesize},
	ob/.style={rectangle,thick,inner sep=0mm, outer sep = 0mm, minimum size=13 mm, align=center, font=\Large, text = black, fill = gray},
	single/.style={->,draw,shorten >=2pt,shorten <=2pt,>=latex', font=\small, gray},
	...
	loading/.style={fill=white,circle,inner sep = 0mm, font = \small, text = gray}
}

\begin{tikzpicture}[scale=1,  decoration = {brace, amplitude = 1.5}]
	
	\coordinate (nodeZero) at (0,0);
\node[ob, right = 1.5mm of nodeZero] (x1) {$x_{1}$};
\node[ob, right = 1.5mm of x1] (x2) {$x_{2}$};
\node[ob, right = 1.5mm of x2] (x3) {$x_{3}$};
\node[ob, right = 1.5mm of x3] (x4) {$x_{4}$};
\node[latent, above = 25mm of $(x1)!0.5!(x4)$] (g) {$g$};
...
\node[error, below = 10 mm of x1] (ex1) {};
\node[error, below = 10 mm of x2] (ex2) {};
\node[error, below = 10 mm of x3] (ex3) {};
\node[error, below = 10 mm of x4] (ex4) {};
...
cat(paste0("$$R_{X}=",bmatrix(drho[1:4,1:4]),"$$"))
```
...
cat(paste0("$$R_{All}=",bmatrix(drho),"$$"))
```
