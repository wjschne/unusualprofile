...
```{r, echo = FALSE, fig.align='center', fig.cap="Figure 1. Test Scores and Estimated Factor Scores of Two Hypothetical Cases"}
...
d_case %>%
  ...
  knitr::kable(
    digits = c(0,0,0,0,1,3,3), 
    align = "lcccccc", 
    caption = "Table 1. Comparing the Unusualness of Two Cases")
```




We can use the proposed method to make sense of observed scores. If the observed scores of the three tests measuring Gc are all 130, we can calculate the (unconditional) Mahalanobis distance to show that the profile of the three scores would be more unusual than about 80% of the population (see Table 1). If the observed scores of the three tests measuring Gc are 115, 130 and 145, the profile would be less usual than 99.8% of population. We can see from this case that although they have equal averages, their unusualness is quite different. 
